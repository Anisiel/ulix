...
st.markdown(
    """
Una raccolta di programmini (in)utili che ho scritto nel tempo, per esigenze d'ufficio.

- Programmini **.bat** per il Prompt dei comandi di Windows (cmd.exe).  
  Si possono eseguire da `cmd` oppure con doppio clic (consigliato inserire `pause` per vedere l’output).
- Uno script **Python (.py)** per il controllo di integrità dei file PDF (richiede Python + dipendenze).
    """,
