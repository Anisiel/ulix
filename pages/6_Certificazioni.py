...
# Config pagina
# ================
st.set_page_config(page_title="Certificazioni")
...
def download_pdf(label: str, filename: str, key: str, primary: bool = False):
    """
    Mostra un download_button usando il percorso assoluto in 'cert/'.
...
with col1:
    with st.expander("EIRSAF Advanced Full", expanded=True):
        ...
        download_pdf("Visualizza badge (PDF)", "Eirsaf_full.pdf", key="eirsaf_full")

with col2:
    with st.expander("EIPASS Progressive Advanced", expanded=True):
        st.markdown("- Certificazione avanzata di competenze digitali\n- Data: 13/02/2025")
        download_pdf("Visualizza badge (PDF)", "Eipass_progressive.pdf", key="eipass_progressive")
...
with col3:
    with st.expander("Certificato livello B2 Inglese", expanded=True):
        ...
        download_pdf("Visualizza badge (PDF)", "Inglese_B2.pdf", key="inglese_b2")
